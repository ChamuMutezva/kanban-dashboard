      ...
      {board.columns.length < 1 ? (
        ...
        <>
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold sr-only">{board.name}</h2>
              ...
              >
                ...
                <div className="space-y-5">
                  {column.tasks?.map((task) => (
                    <InteractiveTaskCard key={task.id} task={task}>
  ...
  return (
    <Card className="rounded-lg shadow-sm border hover:shadow-md transition-shadow">
      <CardHeader>
        ...
        <CardDescription>
          {task.description && <p className="text-sm text-mid-grey mb-3">{task.description}</p>}
              ...
              <ul className="mt-3 space-y-2">
                {task.subtasks.map((subtask) => (
                  <li key={subtask.id} className="flex items-start gap-2">
                    <Checkbox id={subtask.id} checked={subtask.isCompleted} disabled className="mt-0.5" />
                    <label
                      htmlFor={subtask.id}
                      className={`text-sm ${
                        subtask.isCompleted ? "line-through text-mid-grey" : "text-black dark:text-white"
                      }`}
                    >
                      {subtask.title}
                    </label>
