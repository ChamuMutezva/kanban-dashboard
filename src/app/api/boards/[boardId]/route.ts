...
export async function PATCH(request: Request, { params }: { params: { boardId: string } }) {
  try {
    ...
    const updatedBoard = await prisma.$transaction(async (tx) => {
      ...
      const columnIdsToKeep = columns.filter((col) => col.id).map((col) => col.id as string)

      const columnsToDelete = existingColumns.filter((col) => !columnIdsToKeep.includes(col.id))
