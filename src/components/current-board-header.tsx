...
export function CurrentBoardHeader({ boards }: Readonly<{ boards: Board[] }>) {
  ...
  return (
    <div className="flex items-center justify-between w-full pr-4">
      <p className="text-lg sm:text-2xl font-bold">{currentBoard ? currentBoard.name : "Dashboard"}</p>
        ...
        {currentBoard && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={handleEditBoard}>Edit board</DropdownMenuItem>
      ...
      {currentBoard && (
        <EditBoardDialog
          board={{ ...currentBoard, columns: currentBoard.columns || [] }}
          open={editDialogOpen}
      ...
      {/* Delete Board Dialog */}
      {currentBoard && (
        <DeleteBoardDialog board={currentBoard} open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} />
      )}
    </div>
  )
}
